...
def _extract_json_object(raw: str) -> str:
    ...
    prompt instruction — that's what happened 2026-07-02 22:56 UTC (a full
    page of prose, then a well-formed JSON object). Depth-counts braces
    while respecting string literals, so braces nested inside string field
    values (e.g. the "reasoning" text) don't cause premature truncation."""
    start = raw.find("{")
    if start == -1:
        return raw
    depth = 0
